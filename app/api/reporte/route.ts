...
import { VentaPerdida } from '@/lib/types'
...
export async function POST(req: NextRequest) {
  ...
  let records: VentaPerdida[] = readAll()
  ...
  const summary = records.map(r =>
    `[${r.fecha}] ${r.tienda} | Producto: ${r.producto || '?'} | Talla: ${r.talla || '?'} | Comentario: ${r.comentario || '-'}`
  ).join('\n')
...
1. **Productos más demandados** (ranking)
